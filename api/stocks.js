...
async function getFinanceReport(code) {
  try {
    ...
    const eps = row.BASIC_EPS || "-";
    const roe = row.WEIGHTAVG_ROE || row.ROE || "-";
    const grossMargin = row.GROSS_PROFIT_RATIO || "-";
    const netMargin = row.NETPROFIT_RATIO || "-";
...
async function getKlineSimple(code) {
  try {
...
function calcStrategy(basic, klines, finance) {
  ...
  if (finance && finance.available) {
    if (finance.profitYoy > 30 && finance.revenueYoy > 10) {
      chanceScore += 10;
      trendScore += 5;
      buyReasons.push("季度业绩增长较强，基本面有支撑");
    } else if (finance.profitYoy < -20) {
      riskScore += 10;
      riskReasons.push("季度净利润同比下滑，基本面承压");
    }
  }
  ...
  if (riskScore >= 65) {
    ...
    actionAdvice = "趋势、动能和基本面共振较强，已有仓位可持有；无仓不要追高，等回踩5日线或分歧转一致。";
...
async function getStock(stock) {
  ...
  const finance = await getFinanceReport(stock.code);
  const klines = await getKlineSimple(stock.code);
  const strategy = calcStrategy(basic, klines, finance);
